...
in
  ...
  pkgs.stdenv.mkDerivation {
    name = name;
    nativeBuildInputs = with pkgs; [ pkg-config ] ++ pkgs.lib.optionals pkgs.lib.inNixShell [ bear criterion clang-tools ];
    ...
    shellHook = ''
      ...
      echo " > Tip (initial): bear -- make -B -j$(nproc)"
      echo " > Tip (tests): bear --apend -- make -B -j$(nproc) test"
